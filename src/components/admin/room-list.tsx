...
export function RoomList({ view }: { view?: string | null }) {
  ...
  const [successRoom, setSuccessRoom] = useState<{name: string, event: string, code: string} | null>(null);
            ...
            rooms.map((room) => (
              <Card key={room.id} className="group relative overflow-hidden bg-card/30 border-border/50 hover:border-primary/40 backdrop-blur-md rounded-[2.5rem] transition-all duration-500 shadow-xl flex flex-col h-full">
                ...
                <CardHeader className="pb-4 pt-8 px-8 relative z-10">
                  <div className="flex justify-between items-start gap-4">
                    <div className="space-y-1 flex-1">
                      <CardTitle className="text-2xl font-black tracking-tight text-foreground uppercase italic leading-tight group-hover:text-primary transition-colors truncate">
                        {room.name} {room.event_name ? `- ${room.event_name}` : ''}
                      </CardTitle>
                      <div className="flex items-center gap-4">
                         <div className="flex items-center gap-2">
                            <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
                            <span className="text-[10px] font-black uppercase tracking-[0.1em] text-muted-foreground">Active Session</span>
                         </div>
                         {(room as any).event_date && (
                           <div className="flex items-center gap-2 pl-4 border-l border-border/30">
                              <Calendar className="w-3 h-3 text-primary/40" />
                              <span className="text-[10px] font-black uppercase tracking-[0.1em] text-primary/80 italic">
                                {new Date((room as any).event_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                              </span>
                           </div>
                         )}
                      </div>
                    ...
                    <Button 
                      variant="ghost" 
                      size="icon" 
                      className="h-10 w-10 rounded-2xl bg-secondary/50 hover:bg-destructive/10 hover:text-destructive transition-all"
                ...
                <CardContent className="px-8 pb-6 space-y-6 relative z-10 flex-1">
                   ...
                   <div className="space-y-4">
                      <div className="flex items-start gap-3 text-muted-foreground">
                        <Clock className="w-4 h-4 text-primary/60 mt-1" />
                        <div className="flex flex-col gap-2">
                           <div className="flex flex-col">
                              <span className="text-[8px] font-black uppercase tracking-widest text-primary/40">Morning Window</span>
                              <span className="text-[10px] font-black uppercase tracking-widest text-primary/80">IN: {format12h(room.am_time_in_start)} - {format12h(room.am_time_in_end)}</span>
                              <span className="text-[10px] font-black uppercase tracking-widest text-primary/80">OUT: {format12h(room.am_time_out_start)} - {format12h(room.am_time_out_end)}</span>
                           </div>
                           <div className="flex flex-col">
                              <span className="text-[8px] font-black uppercase tracking-widest text-primary/40">Afternoon Window</span>
                              <span className="text-[10px] font-black uppercase tracking-widest text-primary/80">IN: {format12h(room.pm_time_in_start)} - {format12h(room.pm_time_in_end)}</span>
                              <span className="text-[10px] font-black uppercase tracking-widest text-primary/80">OUT: {format12h(room.pm_time_out_start)} - {format12h(room.pm_time_out_end)}</span>
                           </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-3 text-muted-foreground pt-2 border-t border-border/20">
                        <Users className="w-4 h-4 text-primary/60" />
                        <span className="text-xs font-bold tracking-tight">Open Enrollment</span>
                      </div>
                   </div>
                </CardContent>
                ...
                <CardFooter className="px-8 pb-8 pt-0 relative z-10">
                   <Button 
                    variant="secondary" 
                    className="w-full h-12 bg-secondary/80 hover:bg-primary hover:text-primary-foreground rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all shadow-md italic group/btn"
                    onClick={() => handleViewParticipants(room.id)}
                  >
                    <Users className="mr-2 h-3.5 w-3.5 group-hover/btn:scale-110 transition-transform" />
                    Manage Students
                  </Button>
                </CardFooter>
              </Card>
            ...
            <CardHeader className="px-10 pt-10 pb-6 border-b border-border/50">
              <div className="flex items-center justify-between">
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-muted-foreground font-black uppercase tracking-[0.2em] ml-13">Access & Authorization Control</p>
                </div>
                <Button 
                  variant="ghost" 
                  size="icon" 
                ...
                <div className="divide-y divide-border/50">
                  {participants.map((p) => (
                    <div key={p.profiles.id} className="px-10 py-6 flex items-center justify-between hover:bg-primary/5 transition-colors group">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-secondary/50 flex items-center justify-center text-foreground font-black text-sm uppercase italic">
                           {p.profiles.full_name?.[0]}
                        </div>
                        <div className="flex flex-col">
                          <div className="flex items-center gap-2">
                            <span className="font-black text-sm uppercase tracking-tight italic">{p.profiles.full_name}</span>
                            {!p.is_approved && (
                              <div className="px-2 py-0.5 rounded-full bg-yellow-500/10 text-yellow-500 border border-yellow-500/20 text-[8px] font-black uppercase tracking-widest">
                                Pending
                              </div>
                            )}
                          </div>
                          <span className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest leading-none mt-1">
                            {p.profiles.student_id} • {p.profiles.course_year}
                          </span>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        {!p.is_approved && (
                          <Button 
                            variant="ghost" 
                            size="icon" 
                            className="h-10 w-10 rounded-xl bg-green-500/10 text-green-500 hover:bg-green-500 hover:text-white transition-all shadow-lg shadow-green-500/10"
                        ...
                        <Button 
                          variant="ghost" 
                          size="icon" 
                          className="h-10 w-10 rounded-xl bg-destructive/5 text-muted-foreground hover:bg-destructive hover:text-white transition-all shadow-lg shadow-destructive/5"
